...
<div class="tab exchangeGoods_tab">
	...
	<div class="tabBody">
		...
		<textarea id="output_content" name="" cols="40" rows="2" class="textarea_pe" style="padding:5px;font-size:13px;" placeholder="请输入终端号，以逗号(,)、空格或回车分隔"></textarea>
		<br /><br />
		<textarea id="output_checkCode" name="" cols="40" rows="2" class="textarea_pe" style="padding:5px;font-size:13px;" placeholder="终端激活码（无需激活码则不填写），以逗号(,)、空格或回车分隔"></textarea>
	</div>
...
<script type="text/javascript">
	...
	function onOutput() {
		var terminalList = $("#output_content").val().replace(/\r|\n|[ ]/g,",");
		var payChannelId = $("#payChannelSelect").val();
		var checkCode=$("#output_checkCode").val().replace(/\r|\n|[ ]/g,",");
		var reg = /^[0-9a-zA-Z\,]+$/;
