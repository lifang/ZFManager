<tr>
	...
	<td>
		...
		<#if csLease.status=0>待处理
       	<#elseif csLease.status=1>退还中
       	<#elseif csLease.status=2>处理完成
		<#elseif csLease.status=3>已取消
		</#if>
		</strong>
	</td>
	<td id="operation_${csLease.id}">
		<#if csLease.status=0>
			...
			<a class="a_btn" onClick="onHandle(${csLease.id});">标记为处理中</a>
		<#elseif csLease.status=1>
			<a href="<@spring.url "/cs/lease/${csLease.id}/info" />" class="a_btn">查看详情</a>
			<a class="a_btn" onClick="onFinish(${csLease.id});">标记为处理完成</a>
       	<#elseif csLease.status=2>
			<a href="<@spring.url "/cs/lease/${csLease.id}/info" />" class="a_btn">查看详情</a>
		<#elseif csLease.status=3>
			<a href="<@spring.url "/cs/lease/${csLease.id}/info" />" class="a_btn">查看详情</a>
       	</#if>
	</td>
</tr>