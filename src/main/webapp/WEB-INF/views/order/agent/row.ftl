          		...
          		<#if order.status??>
			      	<#if order.status==1><td rowspan="${order.orderGoods?size}"><strong class="strong_status">未付款</strong></td>
						...
						<#if Roles.hasRole("AGENT_ORDER_ADD_PAY_RECORD")><a href="#" class="a_btn paymentRecord_a" onclick="payPriceBtn(${order.id},${(order.actualPrice/100)?string("0.00")});">增加付款记录</a></#if>
	                    		<a href="<@spring.url "/order/agent/${order.id}/info" />" class="a_btn" target="_blank">查看详情</a>
				       ...
				       <#elseif order.status==3><td rowspan="${order.orderGoods?size}"><strong class="strong_status">已发货</strong></td>
						...
						<#if Roles.hasRole("AGENT_ORDER_AGAIN")><a href="<@spring.url "/order/agent/${order.id}/createAgain" />" class="a_btn">再次采购</a></td></#if>
				       <#elseif order.status==4><td rowspan="${order.orderGoods?size}"><strong class="strong_status">已评价</strong></td>
				       		<td rowspan="${order.orderGoods?size}">
				       			<a href="<@spring.url "/order/agent/${order.id}/info" />" class="a_btn" target="_blank">查看详情</a>
						<#if Roles.hasRole("AGENT_ORDER_REMARK")><a href="#" class="a_btn remark_a" onclick="markBtn(${order.id});">备注</a></#if>
						<#if Roles.hasRole("AGENT_ORDER_AGAIN")><a href="<@spring.url "/order/agent/${order.id}/createAgain" />" class="a_btn">再次采购</a></#if>
				       		</td>
				       <#elseif order.status==5><td rowspan="${order.orderGoods?size}"><strong class="strong_status">已取消</strong>
				       	<#if csRefund??>
				       		<#list csRefund as c>
				       			<#list c?keys as key>
				       				<#if key=='orderNumber'>
				       					<#if c[key]==order.orderNumber>
					       					<#if c['status']==1><br><p class="redtext">（退款待处理）</p></#if>
					       					<#if c['status']==2><br><p class="redtext">（退款处理完成）</p></#if>
					       					<#if c['status']==3><br><p class="redtext">（退款已取消）</p></#if>
				       					</#if>
				       				</#if>
				       			</#list>
				       		</#list>
				       	...
				       	<#if cs_refund??>
				       		<#if cs_refund.status==1><br><p class="redtext">（退款待处理）</p></#if>
					       	<#if cs_refund.status==2><br><p class="redtext">（退款处理完成）</p></#if>
					       	<#if cs_refund.status==3><br><p class="redtext">（退款已取消）</p></#if>
				       	</#if>
				       </td>
				       		<td rowspan="${order.orderGoods?size}">
				       			<a href="<@spring.url "/order/agent/${order.id}/info" />" class="a_btn" target="_blank">查看详情</a>
	            ...
	            <#if order.status??>
			      	<#if order.status==1><td><strong class="strong_status">未付款</strong></td>
				       ...
				       <#elseif order.status==2><td><strong class="strong_status">已付款 </strong></td>
				       		<td><a href="<@spring.url "/order/agent/${order.id}/info" />" class="a_btn" target="_blank">查看详情</a>
								<#if Roles.hasRole("AGENT_ORDER_CANCEL")><a href="#" class="a_btn" onclick="cancel(${order.id});">取消</a></#if>
	                            <#if Roles.hasRole("AGENT_ORDER_OUT_STORAGE")><a href="#" class="a_btn deliver_a" onclick="deliverBtn(${order.id},${order.orderGoods?size});">发货</a></#if>
	                            <#if Roles.hasRole("AGENT_ORDER_REMARK")><a href="#" class="a_btn remark_a" onclick="markBtn(${order.id});">备注</a></#if>
           					</td>
				       ...
				       <#elseif order.status==5><td><strong class="strong_status">已取消</strong>
				       <#if csRefund??>
				       		<#list csRefund as c>
				       			<#list c?keys as key>
				       				<#if key=='orderNumber'>
