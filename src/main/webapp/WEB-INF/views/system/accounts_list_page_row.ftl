...
<#if Roles.hasRole("ZF_ACCOUNT_RESET_PWD")><a href="<@spring.url "/system/operate/account/${customer.id}/resetpwd"/>" class="a_btn" target="_blank">密码重置</a></#if>
<#if Roles.hasRole("ZF_ACCOUNT_CREATE_EDIT")><a href="<@spring.url "/system/operate/account/${customer.id}/edit"/>" class="a_btn" target="_blank">编辑</a></#if>
    </td>
</tr>
