...
<script type="text/javascript">
	...
	$(function() {
		...
		$("input[name='re_row']").unbind("click");
		...
		$("#btn_dispatch").bind("click", function() {
			...
			var dispatchIds = [];
			var dispatchedNums = [];
			$("input[name='re_row']").each(function () {
            ...
            if (dispatchedNums.length>0) {
            			$('#dispatch_select').append('<br/><br/><em>所选任务已包含有正在处理中或处理完成，不能分派<em>')
            		}else{
            			 $.get('<@spring.url "/task/refund/dispatch" />',
	            {}, function (data) {
	    ...
	    $("input[name='re_row']").bind("click", function () {
			var total=0;
			var checked=0;
			$("input[name='re_row']").each(function () {
           		total++;
